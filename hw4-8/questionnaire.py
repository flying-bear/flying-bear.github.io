...
def process_form():
    ...
    uid = request.args['uid'] 
    data[uid]['answers'] = OrderedDict(sorted(answers, key=lambda t: t[0]))
    with open('results.json', 'a', encoding='utf-8') as f:
        data[uid] = OrderedDict(sorted(data[uid].items(), key=lambda t: t[0]))
...
def jsonify(): # json со всеми введенными на сайте данными
    ...
    with open('templates/json.html', 'r', encoding='utf-8') as f:
        ...
        table = json2html.convert(jsontext, table_attributes = 'class="form-table", border="1"')
